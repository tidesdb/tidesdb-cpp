...
class TidesDB
{
   ...
   public:
    ...
    int close();
    int create_column_family(const char *name, int flush_threshold, int max_level,
    ...
    int put(const char *column_family_name, const uint8_t *key, const uint8_t *value, time_t ttl);
    int get(const char *column_family_name, const uint8_t *key, uint8_t **value,
            size_t &value_size);
    int delete_key(const char *column_family_name, const uint8_t *key);
    TidesDB();
};
